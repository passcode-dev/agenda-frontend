...
import { useToast } from "@/hooks/use-toast";  // Supondo que você tenha esse hook implementado
...
export default function Agenda() {
    ...
    const handleSaveEvent = () => {
        if (!eventDetails.turmaAluno || !eventDetails.curso || !eventDetails.inicio || !eventDetails.fim) {
            toast({
                ...
                variant: "destructive",
                autoClose: 3000,  // Toast vai desaparecer após 3 segundos
            });
            return;
        }
    ...
    return (
        <div className="flex flex-col items-center p-6 bg-white shadow-lg rounded-xl w-full max-w-5xl transition-all duration-300">
            <div className="flex items-center justify-between w-full mb-4 px-4 transition-transform duration-500">
                <button
                    ...
                    {renderWeek().map((currentDate, dayIndex) => {
                        ...
                        return (
                            <div key={dayIndex} className="border-l border-gray-300 relative">
                                <div className={`p-2 text-center font-semibold ${isToday ? "bg-blue-500 text-white" : "bg-gray-100"} transition-colors duration-300`}>
                                        ...
                                        <div key={hourIndex} className="transition-all duration-300">
                                            ...
                                            {filteredEvents[slotKey] && (
                                                <div
                                                    ...
                                                    style={{
                                                        top: `${parseInt(filteredEvents[slotKey].inicio.split(":")[0]) * 64}px`,
                                                        height: `${(parseInt(filteredEvents[slotKey].fim.split(":")[0]) - parseInt(filteredEvents[slotKey].inicio.split(":")[0])) * 64}px`,
                                                        zIndex: 1,
                                                    }}
                                                >
            ...
            {/* Modal */}
            {isModalOpen && (
                <div className="fixed z-50 inset-0 flex items-center justify-center bg-black bg-opacity-50 transition-all duration-300 opacity-100" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-white p-4 rounded-lg w-80 transition-all duration-500 transform scale-100 opacity-100" onClick={(e) => e.stopPropagation()}>
                        ...
                        <input
                            ...
                            className="w-full p-2 border rounded mt-2 focus:ring-2 focus:ring-blue-500 transition-all duration-300"
                        />
                        <input
                            type="text"
                            placeholder="Curso"
                        ...
                        <button onClick={handleSaveEvent} className="mt-4 p-2 bg-blue-500 text-white rounded transition-all duration-300 transform hover:scale-105">
                            Salvar
                        </button>
                    </div>
                </div>
