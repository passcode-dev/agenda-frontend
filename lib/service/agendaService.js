...
export default class AgendaService {
  ...
  async getSalasByName(name) {
    ...
    async postDiary(payload) {
      ...
      if (response.status === 409) {
        ...
        console.log("Erro de conflito:", errorData.data);
  ...
  async getDiaryTeacher(start, end,id) {
    const response = await httpClient.get(`/diary/teachers?start=${start}&end=${end}&classroom_id=${id}`,true);
    if (response.ok) {
      const jsonData = await response.json();
      return jsonData;
    }
